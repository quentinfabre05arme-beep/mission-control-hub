...
async function main() {
  ...
  console.log(`🏭 Print providers: ${printProviders.length}`);
  
  // Get variants for the first print provider
  const provider = printProviders[0];
  console.log(`   Using: ${provider.title} (ID: ${provider.id})`);
  ...
  // Get first 4 enabled variants (typically S, M, L, XL)
  const enabledVariants = variants.filter(v => v.is_enabled).slice(0, 4);
  console.log(`   Using: ${enabledVariants.map(v => v.title).join(', ')}\n`);
  ...
  for (const design of DESIGNS) {
    ...
    const productData = {
      title: design.title,
      description: design.description || `Premium ${design.title} design`,
    ...
    if (createResult.status === 200 || createResult.status === 201) {
      ...
      console.log(`   ${published ? '✅' : '⚠️'} Etsy: ${published ? 'Published' : 'Pending'}`);
      
      results.push({ title: design.title, id: product.id, price: design.price, published });
    } else {
      console.log(`   ❌ Product failed:`, JSON.stringify(createResult.data).substring(0, 200));
    }
  ...
  console.log(`Etsy: ${results.filter(r => r.published).length} published`);
  
  if (results.length > 0) {
    console.log('\n📦 Products:');
    results.forEach((r, i) => console.log(`  ${i+1}. ${r.title} — $${r.price} ${r.published ? '✅' : '⏳'}`));
    
    fs.writeFileSync('products_published.json', JSON.stringify({ timestamp: new Date().toISOString(), products: results }, null, 2));
    
    console.log('\n🎉 BUSINESS IS LIVE!');
    console.log('🔗 Printify: https://printify.com/app/products');
    console.log('🔗 Etsy: https://quentinvestdesigns.etsy.com');
  }
}
